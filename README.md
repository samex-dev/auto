...
notes for each threshold live in [`CHANGELOG.md`](CHANGELOG.md).
...
| Decide | Possession (network `NetworkOwner`, else carrier/carry/last-shooter), controlled-ball gate (carried ≠ shot), carry correlation, fake/bleed detector, receiver gate (a pass is not a shot), honest wide/over ruling, urgency tiers, per-move counters for ~25 character styles. |
...
* The move counters are name matches against effects the game spawns
  ...
  its name is added to `MOVE_INFO` — the AI prints any `Ichigo<Move>`-style name
  it saw but could not classify.
